...
interface RoomView {
  ...
  players: { userId: string; username: string; color?: "LIGHT" | "DARK"; connected: boolean }[];
  state?: any;
  endVotes?: string[];
  startVotes?: string[];
...
export default function CheckersPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    const s = io(`${API_URL}/checkers`, { auth: { token } });
    socketRef.current = s;
    ...
    s.on("game:state", (r) => { setRoom(r); setSelectedPieceId(null); setLegalDestinations([]); });
    s.on("game:legalMoves", (d) => {
      setLegalDestinations(d.moves.map((m: any) => ({ ...m.destination, captured: m.captured })));
      if (d.mandatoryCaptureElsewhere) {
        setError("Captura obrigatória! Escolha a peça que pode capturar.");
        setTimeout(() => setError(null), 3000);
      }
    });
    s.on("game:finished", (d) => { setWinner(d.winnerUserId); setRoom((r) => (r ? { ...r, state: d.state, status: "FINISHED" } : r)); });
  ...
  function selectPiece(pieceId: string) {
    ...
    socketRef.current?.emit("game:legalMoves", { code: room!.code, pieceId });
  }
  function moveTo(row: number, col: number) {
    if (!selectedPieceId || !room) return;
    socketRef.current?.emit("game:move", { code: room.code, pieceId: selectedPieceId, row, col });
  }
    ...
    return (
      <div>
        ...
        <div className="max-w-md mx-auto px-4 py-16 text-center">
          ...
          <h1 className="text-xl font-semibold text-white mb-1">Damas</h1>
  ...
  if (room.status === "WAITING") {
    return (
      <div>
        ...
        <div className="max-w-md mx-auto px-4 py-16 text-center">
          <div className="text-4xl mb-2">⚪</div>
          <h1 className="text-xl font-semibold text-white mb-1">Sala de Damas</h1>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        {winner && (
          <div className="bg-white text-black rounded-2xl p-4 mb-4 text-center font-semibold">
            🏆 {room.players.find((p) => p.userId === winner)?.username} venceu!
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-white/70 text-sm">{isMyTurn ? "Seu turno" : `Vez de ${room.players.find((p) => p.userId === currentPlayer?.userId)?.username}`}</p>
          <div className="flex gap-2">
            {room.status === "PLAYING" && (
              <>
        ...
        <div className="bg-surface border border-border rounded-2xl p-4">
          <CheckersBoard
            ...
            onSelectPiece={isMyTurn ? selectPiece : () => {}}
